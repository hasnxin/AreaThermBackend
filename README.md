...
in extreme climates (originally built for SIH/DRDO). The frontend prototype
(vanilla HTML/CSS/JS, the original working implementation) lives in the
[AreaTherm](https://github.com/hasnxin/AreaTherm) repo; this repo is the
target production backend described in that project's `ARCHITECTURE.md`.
...
reinterpreted in the port. `DATABASE_SCHEMA.sql`, `ARCHITECTURE.md`, and
`API_SPEC.md` are carried over from the frontend repo as this backend's
...
`/swagger-ui.html`, H2 console at `/h2-console`.
...
output from the frontend's own `engine.js` (winter/summer, 15-min timestep,
PCM thermal mass, round geometry — see `src/test/resources/physics`), and
an optimizer smoke test (567-candidate search, sensitivity analysis, window
...
- No Testcontainers yet (MySQL is verified by hand against a real container
  during development — see commit history in the source repo).
